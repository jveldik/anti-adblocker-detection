...
def create_mlp_model(hp, class_weights):
    model = Sequential()
    ...
    for i in range(num_layers):
        dense_dropout = hp.Choice(f"dense_dropout_{i}", values=[0.2, 0.4, 0.6])
        dense_units = hp.Choice(f"dense_units_{i}", values=[128, 256, 384, 512])
...
def create_model(matrix, labels, set_name, number_of_features, balancing):
    # Reshape labels and matrix for mlp input
    ...
    tuner = Hyperband(
        lambda hp: create_mlp_model(hp, class_weights), 
        objective='val_accuracy',
        overwrite=True,
        directory='data/trials',
        project_name=f"mlp_{balancing}_{set_name}_{number_of_features}"
    )
...
if __name__ == "__main__":
    ...
    for set_name in ["all", "identifier", "literal"]:
        for number_of_features in [100, 1000, 10000]:
            ...
            clf, result = create_model(matrix, labels, set_name, number_of_features, balancing)
            # Save the model
            with open(f"data/models/mlp_{balancing}_{set_name}_{number_of_features}.pickle", 'wb') as f:
                pickle.dump(clf, f)
            # Store the results
            ...
            clf_resampled, result_resampled = create_model(matrix_resampled, labels_resampled, set_name, number_of_features, balancing)
            with open(f"data/models/mlp_{balancing}_{set_name}_{number_of_features}.pickle", 'wb') as f:
    ...
    results_df.to_csv('data/mlp_results.csv', index=False)
